...
beforeEach(async () => {
  ...
  await api
    .post("/api/users")
    .send({ username: "post test", name: "post test", password: "test" });
});
...
describe("addition of a new blog", () => {
  ...
  test("HTTP POST to /api/blogs creates a new blog", async () => {
    ...
    const token = await (
      await api
        .post("/api/login")
        .send({ username: "post test", password: "test" })
    ).body.token;
...
describe("updating blog", () => {
  test("adding a like to a blog changes likes", async () => {
    ...
    await api.put(`/api/blogs/${blogs[0]._id}`).send(updatedBlog);

    const blogsAfterUpdate = await api.get("/api/blogs");
    const blogWithCorrectTitle = blogsAfterUpdate.body.find(
